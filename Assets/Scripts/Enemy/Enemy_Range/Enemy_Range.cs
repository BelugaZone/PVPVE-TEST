...
public class Enemy_Range : Enemy
{
    ...
    protected override void InitializePerk()
    {
        ...
        {
            advanceSpeed = 1;
            SetAnimFloat("AdvanceAnimIndex", 1); // 1 is a slow walk animation
        }
    }
    ...
    public void FireSingleBullet()
    {
        ...
        SpawnLocalBullet(bulletDirectionWithSpread);

        if (base.IsServer)
            RpcSpawnVisualBullet(bulletDirectionWithSpread);
    }

    private void SpawnLocalBullet(Vector3 direction)
    {
        ...
        rbNewBullet.velocity = direction * weaponData.bulletSpeed;
    }

    [FishNet.Object.ObserversRpc(ExcludeServer = true)]
    private void RpcSpawnVisualBullet(Vector3 direction)
    {
        SpawnLocalBullet(direction);
    }
    ...
    {
        if (playerBody == null || player == null) return false;
